...
{
  if (DEBUGG) fprintf (stderr, "tryin %s %6.2f", pt_to_str(str).c_str(), energy/100.0);
  ...
  if (energy == gl_energy) {
    ...
    if (!gl_degen) {
      ...
      enc_tmp.bp_left2=0;
      enc_tmp.bp_right2=0;
      degen_set = find_equal_energy(enc_tmp, energy, deg, gl_direct, degen_lm);
      if (gl_degeneracy) {
